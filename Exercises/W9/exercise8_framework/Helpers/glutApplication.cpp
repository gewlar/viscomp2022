...
void GlutApplication::setOpenGLVersion(int major, int minor, bool useDebugContext /*= false*/)
{
	...
	// no forward compatibility (fragment shader still uses gl_FragColor)
	/*if (useDebugContext) {
...
void GlutApplication::addWindow(GlutWindow *window)
{
    ...
    std::cout << "getWindowID\n";

	setWindowData( window );
    std::cout << "setWindowData\n";
...
bool GlutWindow::getMouseDragStartPositions(int button, int &x, int &y) const
{
	if(button < 0 || button > sizeof(m_mousebuttons)*8)
		return false;
